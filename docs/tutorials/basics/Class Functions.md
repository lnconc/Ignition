...
* **`exposure`:** Defines if the function exists on the `"client"`, `"server"`, or `"shared"`. If the environment doesn't match, the function is never even defined.
...
!!! info "Write-Only Implementation"
    The `object` returned by `import()` is write-only. You can assign functions to it, but you cannot read from it.
``` luau
-- BaseRemote.luau
...
## Static Functions
Static functions belong to the **Class**, not the **Instance**. They do not have a `self` variable and can be called without creating an object first.
