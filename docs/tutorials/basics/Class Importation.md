...
=== "The Destructor: `delete()`"
    ...
    **What `delete()` actually does:**
    ...
    2. **`:Destroy()` Side Effect:** It optionally calls the object's `:Destroy()` if found any.
    3. **State Purging:** It iterates through the object's data rings and calls `free()` on every member. (1)
        { .annotate }
    ...
    4. **Metatable Mutilation and Poisoning:** It replaces all metamethods (like `__index`, `__add`, etc.) with a stub that throws an error if the object is accessed again.
    5. **Closure Erasure:** It wipes all closures associated with the object from registries, namely `env`, `anon`, and `StackSpider`
